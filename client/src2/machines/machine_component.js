...
const MachineSaveFormContent = (props) => {
	const {
		active,
		columns,
		doCommunal,
	...
	return (
		<React.Fragment>
			<div className="w-100 p-1">
				...
				<h3><span className="text-white text-shadow"><FontAwesomeIcon icon="pen" /> Update the &ldquo;{machineName}&rdquo; Machine</span></h3>
					...
					<Form>
						...
						<div className="row mt-3">
							<div className="col-md-12">
								...
								<button type="submit" className="btn btn-success btn-addon" disabled={isSubmitting}><FontAwesomeIcon icon="pen" /> <Translate id="Update the Machine"/> </button>
							</div>
						</div>
					</Form>
				</div>
...
const MachineSaveContent = compose (
	...
	withFormik({
		handleSubmit: async ( input, { props: { MachineSaveMutation, history, setResponse }}, FormikForm ) => {
			...
			if(MachineSave.Response.success === true) {
				setResponse({
					...
					success: true,
					Machine: MachineSave.Machine
				});
			} else {
				setResponse({
					message: "Machine Could Not Be Updated",
					success: false,
					Machine: input
				});
			}
	...
	withTranslate
)(MachineSaveFormContent);

class MachineSaveClass extends React.Component {
	constructor(props) {
    	super(props)
	...
	render () {
		const Machine = this.props.data.Machine || { machineId: 0, active: 1, columns: 1, doCommunal: 1, doIndividual: 1, doPrivate: 1, isMultiChamber: 1, machineName: "", rows: 1 }; // if we don't get back a machine then setup an empty one to use
...
export const MachineSave = compose(
	withRouter,
	queryWithLoading({
        gqlString: getMachineQuery,
        variablesFunction: (props) => ({machineId: props.match.params.machineId}),
